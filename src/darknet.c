...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int min_bb_h, int min_bb_w, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_detector_map(datacfg, cfgfile, weightfile, thresh, iou_thresh, min_bb_h, min_bb_w, 0, net.letter_box, &net_map);// &net_combined);
            printf("\n mean_average_precision (min_bb_h: %d, min_bb_w: %d, mAP@%0.2f) = %f \n", min_bb_h, min_bb_w, iou_thresh, mean_average_precision);
            ...
            for (i = 0; i < nboxes; ++i) {
                ...
                for (class_id = 0; class_id < classes; ++class_id) {
                    ...
                    if (prob > 0) {
                        ...
                        int truth_index = -1; // counter for number of true positives
                        float max_iou = 0; // helping variable to count just the best (best IoU) predicted box for each object the ground truth contains
                        // loop over all objects of the ground truth
                        for (j = 0; j < num_labels; ++j)
                        {
                            box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h };
...
void run_detector(int argc, char **argv)
{
    ...
    else if (0 == strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, thresh, iou_thresh, min_bb_h, min_bb_w, mjpeg_port, show_imgs, benchmark_layers, chart_path);
